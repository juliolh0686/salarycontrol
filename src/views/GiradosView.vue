  ...
  <div class="navbar-central">
    <div>
      <div class="titulo-central">
        Depositos SIAF:
        <span>Ultima Fecha de Actualización: {{ fecha_actualizado }}</span>
      </div>
      <div class="container-from-search">
        ...
        <div>
          <button
            class="btnj btnj-one"
            @click="listarDepositos(0, consulta_dni)"
          >
            Buscar
          </button>
        </div>
        <div class="text-name">
          Apellidos y Nombres: <u>{{ pp_nombre }}</u>
        </div>
      </div>
      <table>
        <thead>
        ...
        <tbody>
          <tr v-for="(personal, index) in lista_personal" :key="personal.pm_id">
            ...
            <td>
              <ul
                v-for="detalledoc in personal.detalledoc"
                :key="detalledoc.ded_id"
...
const listarDepositos = async (page: number, dni: string) => {
  ...
  if (response.status == false) {
    ...
    fecha_actualizado.value = response.fecha_actualizado;
